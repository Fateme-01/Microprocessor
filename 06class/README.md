...
در این تابع، ملودی به ترتیب اجرا می‌شود:
    ...
    تعیین مدت زمان هر نت: مدت زمان نت‌ها با استفاده از 1000 / noteDurations[thisNote] محاسبه می‌شود، که در آن thisNote شماره‌ی نت فعلی است.
    پخش نت با tone(): تابع tone(8, melody[thisNote], noteDuration) فرکانس نت مشخص شده در melody را برای مدت زمان مشخصی که در noteDuration ذخیره شده، به بیزر ارسال می‌کند.
    توقف بین نت‌ها: پس از پخش هر نت، یک تاخیر 30 میلی‌ثانیه‌ای برای جدا کردن نت‌ها اعمال می‌شود.
